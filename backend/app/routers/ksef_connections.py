...
@router.post("", response_model=KsefConnectionResponse)
...
@router.get("", response_model=List[KsefConnectionResponse])
def list_ksef_connections(
    db=Depends(get_db),
    current_user=Depends(get_current_user)
):
    return get_ksef_connection(db=db)

@router.get("/{connection_id}",
    response_model=KsefConnectionResponse
)
def get_single_ksef_connection(
    connection_id: int,
    db=Depends(get_db),
    current_user=Depends(get_current_user)
):
